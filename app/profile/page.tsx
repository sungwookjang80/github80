...
export default async function ProfilePage() {
  ...
  const [{ data: topics }, { count: convCount }, { count: msgCount }] = await Promise.all([
    supabase.from('topics').select('*').eq('user_id', user.id).order('count', { ascending: false }),
      ...
      <main className="max-w-2xl mx-auto p-6 space-y-6">
        <div className="grid grid-cols-2 gap-4">
          <div className="bg-white border border-gray-200 rounded-xl p-4 text-center">
            <p className="text-3xl font-bold text-indigo-600">{convCount ?? 0}</p>
            <p className="text-sm text-gray-500 mt-1">총 대화 수</p>
          </div>
          <div className="bg-white border border-gray-200 rounded-xl p-4 text-center">
            <p className="text-3xl font-bold text-indigo-600">{msgCount ?? 0}</p>
