  ...
  return (
    <form onSubmit={handleSubmit} className="flex gap-2 items-end p-4 border-t border-gray-200 bg-white">
      <textarea ref={ref} value={value} onChange={e => setValue(e.target.value)} onKeyDown={handleKeyDown}
        ...
        className="flex-1 resize-none border border-gray-300 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-sand disabled:opacity-50 max-h-32" />
      <button type="submit" disabled={!value.trim() || disabled}
        className="bg-sand text-white rounded-xl px-4 py-3 text-sm font-medium hover:bg-sand-700 disabled:opacity-40 shrink-0">
        전송
      </button>
    </form>
  )
